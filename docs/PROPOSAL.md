...
minutes, ships with its own tests, and a self-playing explainer film - generated by
the product, assuming no prior context - is at `/film.html`. This proposal grew out
of a broader agentic-banking build; everything that wasn't this one idea was
deliberately cut, because the idea deserves a repository with nothing else in it.*
