...
Built to run identically on any machine: Windows, macOS, Linux, a fresh VM, a judge's
laptop. If port 8000 is already taken by something else on that machine (common - lots
of dev tools default to it), the server automatically tries the next few ports and
tells you which one it's actually using, instead of crashing.

```bash
python3 app.py           # → http://localhost:8000  (`python app.py` on Windows)
python3 test_engine.py   # 38 checks over the real logic
python3 test_server.py   # 16 checks: the security attack pass, codified
```
...
**The film:** [http://localhost:8000/film.html](http://localhost:8000/film.html) - a
self-playing explainer that assumes nothing: it defines DBT, KYC and Aadhaar in plain
language, walks through the mechanism (the self-healing name match, the deterministic
gate, the dropped-call memory), and grades the product's own honesty - all narrated
live by the same voice engine the product uses (English scenes, and the agent's line
in Hindi), with a couple of its numbers fetched live from this server's own engine so
they can never be scripted. It isn't a rendered video file - it's *generated by the
product each time*, so it can never go stale. To produce an `.mp4` for a submission
portal, screen-record one play-through.
...
├── test_engine.py   38 checks over the real logic
├── test_server.py   16 checks: the security attack pass, codified
├── web/
│   ├── index.html   the narrative + the district queue + the interactive rescue
│   ├── film.html    the self-playing explainer film (the product narrates itself,
│   │                from scratch, with live numbers fetched from its own engine)
